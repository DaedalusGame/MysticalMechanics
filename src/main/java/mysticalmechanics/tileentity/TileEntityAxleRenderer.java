    ...
    public void render(TileEntityAxle tile, double x, double y, double z, float partialTicks, int destroyStage, float tileAlpha){
        if (tile != null){
            ...
            if (state.getBlock() instanceof BlockAxle){
                ...
                TileEntityAxle leftAxle = tile.getConnectionTile(EnumFacing.AxisDirection.NEGATIVE);
                TileEntityAxle rightAxle = tile.getConnectionTile(EnumFacing.AxisDirection.POSITIVE);
                double angle = 0;
                double lastAngle = 0;
                if(leftAxle != null && rightAxle != null) {
                    double leftPower = leftAxle.capability.getPower(null);
                    double rightPower = rightAxle.capability.getPower(null);
                    if(leftPower > rightPower) {
                        angle = leftAxle.angle;
                        lastAngle = leftAxle.lastAngle;
                    }
                    else {
                        angle = rightAxle.angle;
                        lastAngle = rightAxle.lastAngle;
                    }
                }
